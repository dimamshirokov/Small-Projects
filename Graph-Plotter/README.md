<h1 align='center'> Graph Plotter </h1>

<p align='center'> <img src='https://drive.google.com/uc?id=1Xj7545oU7rPrfZxhh2zJjzSKibTJr_OM' width='70%'> </p>

___

## 📋 Description

This is a desktop application that allows you to plot functions from one variable over a given period. 
You can also save the image of this graph in .png format.

## 🏋🏼‍♀️ Requirements

- tkinter
- numpy
- matplotlib

## 🧩 Run the program

While in the folder /Small-Projects/Graph-Plotter, run the command below.

```console
pip install -r requirements.txt
```

Meanwhile, in the folder /Small-Projects/Graph-Plotter/src, run the command below.

```console
python3 main.py
```

## 📉 Portfolio

Examples of graphs of functions constructed using the Graph-Plotter application.

![](https://drive.google.com/uc?id=1pqPRM1YA9o6hGQtxx_Akn6ArhIAa-kuB)

![](https://drive.google.com/uc?id=1Qs26z9qMM99N3lDQY1GWnZzmmrygV3Jw)

![](https://drive.google.com/uc?id=1ApvAXgIKO5NN8r8d1P_9kNdJRaP-bkL6)

![](https://drive.google.com/uc?id=1ODOwvYjbYy0YnsQFIZQ72QXFj3cZ_tBV)

## :octocat: Author

**Dima M. Shirokov**
- [GitHub](https://github.com/dimamshirokov)